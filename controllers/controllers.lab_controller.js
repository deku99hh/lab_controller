...
let io; // تعريف السوكيت في نطاق الملف
...
const shutdownAPC = (req, res) => {
    ...
    if (io) {
        if (pcId === 'all') {
            io.to(`lab_${lab}`).emit('force_shutdown', { message: 'Server forced shutdown' });
        } else {
            io.to(`lab_${lab}_pc_${pcId}`).emit('force_shutdown', { message: 'Server forced shutdown' });
        }
...
const shutdownAllowence = async (req, res) => {
    ...
    try {
        const clintInDB = await PcsData.findOne({ pcNumber, labNumber, macAddress });

        if (!clintInDB) {
            return res.status(404).send({ error: 'PC not found' });
        }

        if (!clintInDB.openTimeAllowed || clintInDB.openTimeAllowed < now) {
            res.send({ states: true });
        } else {
            res.send({ states: false, timeRemaining: (clintInDB.openTimeAllowed - now) });
        }
    } catch (err) {
...
const clintAllowanceChange = async (req, res) => {
    ...
    try {
        await PcsData.updateOne(
            { pcNumber, labNumber }, 
            { openTimeAllowed: new Date(openTimeAllowed) }
        );
        res.send('done');
    } catch (err) {
        res.status(500).send('Error updating allowance');
    }
};
...
// التأكد من تصدير كل الدوال بشكل صحيح
module.exports = {
    setSocketIo,
    starting,
    updateDatabaseClint,
